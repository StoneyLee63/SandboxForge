...
**Why it matters:** Phase 1 proved the routing. This phase proves the first real operation the tool performs, and every later phase depends on it. `enter` and `destroy` are meaningless without a container that reliably exists and can be found by name. It is also where the tool stops being a script that runs on one machine in one directory and becomes something that runs anywhere.
...
Both were addressed in this phase. The second was found the hard way.
...
1. **Wrote the image definition.** Created `Dockerfile` at the project root, deliberately beside `bin/` rather than inside it, so the build context is the repository itself. Two instructions: `FROM ubuntu:24.04` to establish the base filesystem, and `CMD ["sleep", "infinity"]` to define the container's main process.

2. **Chose `sleep infinity` over `/bin/bash` for `CMD`.** A container lives exactly as long as its main process. With no terminal attached, `bash` finds nothing to do and exits immediately, killing the container in the same second it started. `enter` in Phase 3 attaches to an already-running container, so the container must stay up after `create` returns. A process that never finishes is what keeps it there.

3. **Built the image manually before scripting it.** Ran `docker build -t sandboxforge-base:latest .` directly to confirm the Dockerfile was valid in isolation, the same separation-of-failure-domains discipline used in Phase 1. Verified the tag in the build output (`naming to docker.io/library/sandboxforge-base:latest`) rather than assuming it.

4. **Built `create_sandbox()` incrementally.** Defined the function above `main()` so it exists before `main "$@"` invokes it at the bottom of the file. Added `local name="$1"`, which is the function's own first positional parameter and independent of the script's, then `local image="sandboxforge-base:latest"` matching the tag used in the manual build.
...
9. **Added two guard clauses at the top of the function**, one rejecting an empty name and one rejecting a name already in use, placed above the build so that no expensive work runs before cheap validation.

10. **Reversed the build strategy on evidence** after an offline failure (documented below), wrapping the build in `if ! docker image inspect "$image" >/dev/null 2>&1`.

11. **Reconciled the README with the implementation** after finding that it documented a profile system which does not exist, then committed the checkpoint.
...
The original design called `docker build` unconditionally, on the reasoning that layer caching made rebuilds nearly free. That was measured at 2.0 seconds against 17 seconds cold, so the reasoning was sound about CPU cost and silent about connectivity. The machine was offline, and offline is precisely the condition a portable sandbox tool is supposed to survive.
...
**Bug 3 — a guard clause that failed open.** The duplicate-name check was written as `docker ps -a --format '{{.Name}}'`. Docker's container context exposes `Names`, plural. There is no `Name` field, and the template engine errored with `can't evaluate field Name in type *formatter.ContainerContext`.

The important part is what happened next. The failing `docker ps` produced no output, `grep` therefore found no match and returned non-zero, the `if` read that as "no duplicate exists," and execution continued straight into the build and the container start, which then failed with Docker's own `Conflict` error. The guard blocked nothing and never announced that it had broken.
...
- **A container lives exactly as long as its main process.** `CMD` is not configuration. It is the process the container exists to run, and choosing one that exits is choosing a container that dies.
...
- **Documentation is verified against the implementation, not against intent.** The README described a profile system that was never built. On a public repository, documentation that does not match the code is worse than none, because it fails for the reader on their first attempt to use the tool.
...
**Named resources are auditable. Generated identifiers are not.** `--name` gives every sandbox a deterministic handle that a human can reason about and a later action can target. Phase 5's audit log depends on it, because a log of container IDs describes what happened without describing what it was for.
...
**`docker image inspect`** — reports metadata for an image in the local image store. Succeeds only if the image is present locally, and makes no network request.
...
**Tag** — a human-readable label pointing at an image, written `name:tag`. Tags are mutable and can be moved to a different image, while the image itself is unchanged.
...
**`>/dev/null 2>&1`** — discards a command's output entirely. `>/dev/null` sends stdout to the null device, and `2>&1` redirects stderr to wherever stdout is currently going.
